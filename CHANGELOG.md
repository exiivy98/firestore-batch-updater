...
- **`updateField(field, value, options?)` method** - Update a single field on all matching documents
  - Convenience wrapper around `update()` for single-field updates
  - Supports dot notation for nested fields (e.g., `settings.theme`)
  - Works with `FieldValue` helpers (`increment()`, `arrayUnion()`, `serverTimestamp()`, etc.)
  - Supports all `update()` options: `dryRun`, `batchSize`, `onProgress`, `log`
...
- **`fieldStats(field)` method** - Get combined statistics (sum, avg, min, max, count) for a single field in one call
  - Runs aggregate, min, and max queries in parallel for efficiency
  - Useful for dashboards and analytics views that need multiple stats at once
  - Supports `where()` filtering
  - Returns `FieldStatsResult` with all five values

### Types

- Added `FieldStatsResult` type

## [1.16.0] - 2026-04-20

### Added

- **`pluckIds()` method** - Get an array of document IDs from matching documents
  - Simpler alternative to iterating over `getAll()` results for IDs
  - Useful for chaining with `bulkUpdate()`, `bulkDelete()`, etc.
  - Works with `where()`, `orderBy()`, `limit()` for filtered/sorted ID extraction
...
- **`sample(n)` method** - Get a random sample of matching documents
  - Returns n randomly selected documents with `{ id, data }`
  - Works with `where()` and `select()` for filtered/partial sampling
  - Returns all documents if n is greater than total count
...
- **`sum()` method** - Get the sum of a numeric field from matching documents
  - Convenience wrapper around `aggregate()` for simple sum queries
  - Returns `number | null` directly (no need to define alias or spec)
  - Works with `where()` for filtered sums
- **`avg()` method** - Get the average of a numeric field from matching documents
  - Convenience wrapper around `aggregate()` for simple average queries
  - Returns `number | null` directly (no need to define alias or spec)
  - Works with `where()` for filtered averages
